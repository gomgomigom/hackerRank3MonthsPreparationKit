function gamingArray(arr) {
    var max = 0;
    var move = 0;
    arr.forEach(function (value) { return (value > max ? ((max = value), move++) : null); });
...
function stringToNumber(str) {
    var numberArray = [];
    str.split(' ').forEach(function (value) { return numberArray.push(parseInt(value)); });
    return numberArray;
}
function gamingArray3(arr) {
    var m = 0;
    return arr.reduce(function (c, v) { return (v > m ? ((m = v), c + 1) : c); }, 0) % 2
        ? 'BOB'
        : 'ANDY';
}
